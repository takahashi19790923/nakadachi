...
| 発信者情報 | 183日 | 記録 |
| **決済記録** | **7年（帳簿書類）** | 作成 |
| 監査ログ（管理操作・決済・退会） | 消さない | — |
| 監査ログ（ログインの成功・失敗 `auth.*` / `authz.*`） | **180日** | 記録 |
...
手元に空の PostgreSQL（PGlite）を立て、器を作り、写しを流し込み、
件数と**書き込めること**まで確かめて、所要時間を出します。
**Docker も外部のDBも要りません。** 2026-08-26 に実施済み（1.2秒 / 266行）。

これで分かるのは「**手順が最後まで通ること**」と
「**実データ特有の問題が無いこと**」です。
分からないのは**東京への往復時間**だけ。下はそれを測りたい場合です。

##### 往復時間まで測る場合：使い捨ての Supabase

> **★無料枠では作れません。★** 上限はアカウントあたり2プロジェクトで、
> 本番2つ（nakadachi / vtuber-sns）で埋まっています。
> **新しい組織を作っても増えません**（2026-08-26 に実際に試して確認）。
> Pro の一時利用が要ります。

使い捨ての Supabase プロジェクトへ本番の写しを戻し、
**所要時間を測って**下の表に書きます。
...
| **2026-08-26** | **手元（PGlite）** | `db/2026-08-25.json`（本番の写し） | **266** | **1.2秒**（流し込み 0.1秒） | h.takahashi | `pnpm run db:drill`。**本物の本番データで一往復**。件数一致・**書き込みできること**・生成列が計算し直されること（2件中0件が空）まで確認 |
| — | 東京の Supabase への往復時間 | | | **未計測** | | ★使い捨てプロジェクトが要る。無料枠はアカウントあたり2つで、本番2つ（nakadachi / vtuber-sns）で埋まっている（2026-08-26 確認。**新しい組織を作っても増えない**）。Pro を一時利用するなら測れる★ |

**いまの規模なら、復旧そのものは一瞬です**（266行・71KB）。
効いてくるのは**人の手順のほう**なので、迷わず進める状態にしておくことが目的です。
行数が万の単位になったら測り直してください。
