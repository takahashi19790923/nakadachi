...
## 2-2. 開示請求・捜査関係事項照会が来たとき

> **★本番へ直接つないで、その場でコードを書かないこと。★**
> プライバシーポリシーで「参照した事実は記録に残します」と約束しています。
> 直接つなぐと、その参照が**いちばん記録に残らない形**で行われます。
> 画面（`/admin/disclosure`）を使えば、記録は自動で残ります。

### 本人からの請求（個人情報保護法33条・34条）

1. **本人であることを確かめる。** 登録されているアドレスへこちらから連絡し、
   そのやり取りの中で応じる。返信先を請求書面に書かれたアドレスにしない
   （なりすましの一番安い経路）
2. 内容に応じて対応する

| 求められたもの | どこを見るか |
| --- | --- |
| 登録情報の開示 | 管理画面の利用者一覧。**メールアドレスは暗号化されている**ので、必要なら `scripts/` から復号する |
| 投稿・メッセージの開示 | 管理画面（投稿一覧・通報対応） |
| 訂正・追加 | 本人にマイページから直してもらう |
| 削除・利用停止 | **退会**の案内（30日後に削除）。決済記録は7年の保存義務があるため、個人が特定できない形にして残る |

3. **遅滞なく回答する。** 法律に日数の定めはありませんが、
   2週間を超えるなら途中経過を知らせる

### 発信者情報の開示・捜査関係事項照会

**`/admin/disclosure` を使う。** 3層すべてを通り、理由の入力が必須で、
**引いた事実は復号より先に記録されます**（消せません）。

1. 書面を確認する。**照会の根拠（条文・事件番号・発出元）を控える**
2. `/admin/disclosure` で、利用者 ID か対象（投稿・メッセージ・通報）の ID から引く
3. 理由欄に、上で控えた根拠をそのまま書く
4. 必要な範囲だけを回答する。**求められていない期間・対象まで渡さない**

> **保存期間は183日です。** それより古いものは残っていません。
> 「記録が無い」ことも回答として成立します。無理に探さない。

> **判断に迷ったら、答える前に止まる。** 一度渡した情報は取り消せません。
> 弁護士等に相談する時間を取ってください。
