...
| DB | **本番: Supabase PostgreSQL（東京）**、preview/dev: Neon PostgreSQL。本番・preview は **Cloudflare Hyperdrive 経由**（`pg` 8.23）、ローカル・テストは `@neondatabase/serverless` 1.1.0 で直接 | — |
...
| 名前 | 用途 |
|---|---|
| `DATABASE_URL` | Neon の pooled 接続文字列。★本番・preview では Hyperdrive の binding があれば使われない★（無いときの退避路。本番の実体は Supabase なので、この退避路は Neon を消した時点で死ぬ。DEPLOYMENT.md「DB は Supabase」） |
