...
> **★2026-08-29 現在、デプロイは «手動» です。★**
>
> ここには「Cloudflare 側と GitHub の連携で行う」と書いてありましたが、
> **Workers Builds は接続されていません**（ダッシュボードの
> Settings → ビルド は「Git リポジトリ」の接続ボタンが出たままです）。
> 実際のデプロイは、手元からこれを打っています。
>
> ```bash
> pnpm run deploy:production
> ```
>
> **書いてあることと、していることが違う状態でした。**
> `main` にマージしても何も出ません。出すには上を打ちます。
>
> `main` にはブランチ保護をかけたので（検査5件が必須・強制 push 不可・
> 管理者にも適用）、`main` に入っているものは必ず検査を通っています。
> あとは「出す」を人が決めるだけ、という形です。

**自動デプロイにしたい場合**（任意）は下の手順で接続します。
...
> **★`main` のブランチ保護（2026-08-29 に設定済み）★**
>
> | 設定 | 値 |
> |---|---|
> | 必須の検査 | `lint / typecheck / test / build` / `E2E` / `秘密情報の混入確認` / `依存の脆弱性確認` / `既知の脆弱性` |
> | 最新への追従 | 必須 |
> | 強制 push | 不可 |
> | ブランチの削除 | 不可 |
> | **管理者にも適用** | **する** |
>
> **★`enforce_admins` を立てないと、所有者は素通りします。★**
> 最初 false のまま設定して直接 push を試したら、`Bypassed rule violations`
> と出て**通ってしまいました**。1人のリポジトリでは所有者＝唯一の利用者なので、
> これが無いと保護は飾りです。立てたあとは `GH006` で弾かれることを確認済み。
>
> 緊急時に直接 push したいときは、GitHub の Settings → Branches から
> 一時的に外してください（画面に操作の跡が残ります）。
